...
tx_pool_sync_abort(PeerId) ->
    call(PeerId, {tx_pool, [sync_abort]}).
...
map_request(txps_abort)           -> tx_pool;
...
handle_msg(S, MsgType, false, Request, {ok, Vsn, Msg}) ->
    case MsgType of
        ...
        txps_abort           -> handle_tx_pool_sync_abort(S, Msg)
    end;
handle_msg(S, MsgType, true, Request, {ok, _Vsn, Msg}) ->
    case MsgType of
        ...
        txps_abort    -> handle_tx_pool_sync_rsp(S, abort, Request, Msg)
...
handle_tx_pool_sync_init(S, _MsgObj) ->
    lager:debug("ZZZ got sync_init", []),
    case aec_tx_pool:sync_init(peer_id(S)) of
        ok ->
            lager:debug("ZZZ send sync_init ok", []),
            send_response(S, txps_init, {ok, #{}});
        Err = {error, _} ->
            lager:debug("ZZZ send sync_init error ~p", [Err]),
...
handle_tx_pool_sync_unfold(S, MsgObj) ->
    ...
    case aec_tx_pool:sync_unfold(peer_id(S), Unfolds) of
        {ok, NewUnfolds} ->
...
handle_tx_pool_sync_get(S, MsgObj) ->
    ...
    case aec_tx_pool:sync_get(peer_id(S), TxHashes) of
        {ok, Txs} ->
...
handle_tx_pool_sync_abort(S, _MsgObj) ->
    aec_tx_pool:sync_abort(peer_id(S), {error, none}),
    send_response(S, txps_abort, {ok, #{}}),
    S.

handle_tx_pool_sync_rsp(S, Action, {tx_pool, From, _TRef}, MsgObj) ->
    lager:debug("ZZZ tx_pool rsp ~p", [Action]),
    case Action of
        init ->
