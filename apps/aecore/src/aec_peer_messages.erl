...
serialize(txps_abort, _TxpsAbort, Vsn = ?TX_POOL_SYNC_ABORT_VSN) ->
    serialize_flds(txps_abort, Vsn, []);
...
tag(txps_abort)           -> ?MSG_TX_POOL_SYNC_ABORT.
...
rev_tag(?MSG_TX_POOL_SYNC_ABORT)   -> txps_abort.
...
latest_vsn(txps_abort)           -> ?TX_POOL_SYNC_ABORT_VSN.
...
deserialize(txps_abort, Vsn, TxpsAbortFlds) when Vsn == ?TX_POOL_SYNC_ABORT_VSN ->
    [] = aec_serialization:decode_fields(
             serialization_template(txps_abort, Vsn), TxpsAbortFlds),
    {txps_abort, Vsn, #{}};
...
serialization_template(txps_get, ?TX_POOL_SYNC_GET_VSN) ->
    [{tx_hashes, [binary]}];
serialization_template(txps_abort, ?TX_POOL_SYNC_ABORT_VSN) ->
    [];
