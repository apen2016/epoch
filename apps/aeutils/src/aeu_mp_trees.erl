...
-spec unfold(path(), hash(), tree()) -> term().
unfold(<<>>, Hash, #mpt{ hash = Hash }) ->
    [];
unfold(<<>>, _, #mpt{ hash = Hash, db = DB }) ->
    int_unfold(<<>>, decode_node(Hash, DB), DB);
unfold(Prefix, Hash, #mpt{ db = DB }) ->
    int_unfold(Prefix, decode_node(Hash, DB), DB).

-spec has_node(path(), enc_node(), tree()) -> yes | no | partially.
has_node(Path, Node, T = #mpt{ hash = Root, db = DB }) when byte_size(Node) < 32 ->
    case decode_node(Node, DB) of
        {'leaf', NodePath, Val} ->
            case get(<<Path/bits, NodePath/bits>>, T) of
                Val -> yes;
                _   -> no
            end;
        {'ext', NodePath, NodeVal} ->
            has_node(<<Path/bits, NodePath/bits>>, NodeVal, T);
        {branch, _} ->
            int_has_node(Path, Node, decode_node(Root, DB), DB)
    end;
has_node(Path, Node, #mpt{ hash = Root, db = DB }) ->
...
int_unfold(Prefix, {'leaf', Key, _}, _DB) ->
    [{key, <<Prefix/bitstring, Key/bitstring>>}];
